<template>
  ...
  <div :class="computeFullScreenClass">
    <header-menu :tag-edit-check="tagEditCheck"></header-menu>
    <main-editor @afterMount="$emit('afterMount')" :editNodeFn="editNodeFn"></main-editor>
    <navigator></navigator>
...
export default {
  ...
  computed: {
    ...mapGetters("caseEditorStore", ["config", "minder"]),
  ...
  methods: {
    ...
    filterMindForLabel(priority, testResult, source) {
      ...
      function filter(data) {
        ...
        const matchPriority =
          hasPriorityCondition && priority.indexOf(data.data.priority) !== -1;
        const matchTestResult =
          hasTestResultCondition &&
          testResult.indexOf(data.data.testResult) !== -1;
        const matchSource = hasSourceCondition && intersection.length !== 0;
        needExpand =
          (!hasPriorityCondition || matchPriority) &&
          (!hasTestResultCondition || matchTestResult) &&
          (!hasSourceCondition || matchSource);
        if (needExpand) {
    ...
    reloadModules(config) {
      ...
      if (!config.allowEditLabel) {
        // 修改自定义标签开关
        disableModuleNames.push("ResourceCommand");
      }
      if (!config.allowEditResult) {
        // 修改测试结果开关
      ...
      if (!config.allowEditNode) {
        ...
        // 关掉右键展示 hotbox 功能
        if (
          editor.hotbox.$element.parentNode === window.editor.hotbox.$container
        ) {
